...
- **Contact/support data handling**: The only public contact channel is `mailto:calceditorapp@gmail.com` (see `docs/SITE_ROADMAP_C_PROJECT_IDENTITY.md`). The Site does not currently collect form submissions, so there is no server-side contact-data handling to govern.
